...
def prac7ConvMLPModel(model='MLP',MLPTop={},convTop={},optimiser={},
  ...
  if model=='convNet':
    ...
    for i in range(topology.pop('convPoolLayers')):
      if i==0:
        layers[str(i)] = Helpers.convLayer(image_shaped_input,"convPoolLayer"+str(i),i,**topology)
      else:
        layers[str(i)] = Helpers.convLayer(layers[str(i-1)],"convPoolLayer"+str(i),i,**topology)
    ...
    for i in range(len(hiddenDims)):
      if i==0:
        layers[str(i)] = Helpers.FCLayer(x, 784, hiddenDims[i],"hidden_layer_"+str(i))
      else:
        layers[str(i)] = Helpers.FCLayer(layers[str(i-1)],hiddenDims[i-1],hiddenDims[i],"hidden_layer_"+str(i))
  ...
  testPath,trainPath = Helpers.getSaveDir(model)
